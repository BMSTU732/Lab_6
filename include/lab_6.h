...
public:
	...
	Ring operator+ (const Ring& a) //ïåðåãðóçêà îïåðàòîðà +
	{
		Ring result(a.r + r);
		return result;
	}
...
public:
	...
	void print_matrix() //ïå÷àòü ìàòðèöû
	{
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < m; j++)
	...
	void print_ptr_matrix() //ïå÷àòü ìàòðèöû
	{
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < m; j++)
	...
	Ring& index(int a, int b)//îáðàùåíèå ïî ýëåìåíòó
	{
		return matrix[a][b];
	}
	void setplus(T** sum)
	...
	Matrix<T> operator+ (Matrix<T> & matrix2) //ïåðåãðóçêà îïåðàòîðà +
	{
		T **sum = new T *[n];
		for (int i = 0; i < n; i++)
		{
	...
	Matrix <T> operator < (Matrix<T> &matrix2) //ïåðåãðóçêà îïåðàòîðà òðàíñïîíèðîâàíèÿ
	{
		Matrix sum(*this);
		for (int i = 0; i < n; i++)
		{
...
int main()
{
	srand(time(NULL));
	Matrix<int> mat1;
	mat1.print_matrix(); //ïå÷àòü ìàòðèöû
